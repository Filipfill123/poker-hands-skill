...
class PokerHands(MycroftSkill):
    def __init__(self):
        ...
        self.STATE_REPRESENTATION = StateRepresentation(self.skill_name, self.no_of_slates, False)
    ...
    def handle_hands_poker(self, message):
        ...
        if (first_card not in self.cards) and (second_card not in self.cards): # both card were misunderstood
            ...
            self.STATE_REPRESENTATION.set_slot_value_not_cofirmed(first_card,1)
    ...
    def handle_first_card_hands_poker(self, message):
        first_card = message.data.get('first_card')
        if self.STATE_REPRESENTATION.get_slot_value(1) is not None:
    ...
    def handle_kill(self, message):
        if self.STATE_REPRESENTATION.get_slot_value(1) is None and self.STATE_REPRESENTATION.get_slot_value(2) is None:
        ...
        elif self.STATE_REPRESENTATION.get_slot_value(1) is None and self.STATE_REPRESENTATION.get_slot_value(2) is not None:
